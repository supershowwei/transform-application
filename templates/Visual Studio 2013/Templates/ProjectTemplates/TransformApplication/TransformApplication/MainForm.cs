...
namespace $safeprojectname$
{
    public partial class MainForm : Form
    {
        ...
        private AutoResetEvent workingMessageWait = new AutoResetEvent(false);
        private bool stopWorkingMessageHandler;
        ...
        public MainForm(string[] args)
        {
            ...
            this.WorkingMessageQueue = new ConcurrentQueue<WorkingMessage>();
            this.WorkingMessages = new BindingList<WorkingMessage>();
            this.ErrorMessages = new BindingList<WorkingMessage>();
        ...
        private ConcurrentQueue<WorkingMessage> WorkingMessageQueue { get; set; }

        private BindingList<WorkingMessage> WorkingMessages
        {
            get { return (BindingList<WorkingMessage>)MessageGridView.DataSource; }
            set { MessageGridView.DataSource = value; }
        }

        private BindingList<WorkingMessage> ErrorMessages
        {
            get { return (BindingList<WorkingMessage>)ErrorGridView.DataSource; }
            set { ErrorGridView.DataSource = value; }
        }
        ...
        {
            this.WorkingMessageQueue.Enqueue(new WorkingMessage(DateTime.Now, logLevel.DisplayName, logMessage));

            this.workingMessageWait.Set();
        }

        private void WorkingMessageHandler()
        {
            this.stopWorkingMessageHandler = false;
            while (true)
            {
                WorkingMessage message;
                if (this.WorkingMessageQueue.TryDequeue(out message))
                {
                    this.InvokeIfNecessary(() =>
                    {
                        ...
                        if (this.AutoScrollCheckBox.Checked)
                        {
                            ...
                            if (this.MessageAppearance.MaxRows > 0)
                            {
                                while (this.WorkingMessages.Count > this.MessageAppearance.MaxRows)
                                {
                                    this.WorkingMessages.RemoveAt(0);
                                }
                            }
                ...
                {
                    this.workingMessageWait.WaitOne();
                }

                if (this.stopWorkingMessageHandler)
                {
                    break;
                }
            }
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            // 開始聽訊息
            Task.Run(() => { this.WorkingMessageHandler(); });
        }
        ...
        {
            switch (((WorkingMessage)((DataGridView)sender).Rows[e.RowIndex].DataBoundItem).Status)
            {
                case "WARN":
        ...
        private void ErrorGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 0)
            {
                ...
                {
                    Log.Info("使用者處理了錯誤訊息：" + this.ErrorMessages[e.RowIndex].Message);

                    this.ErrorMessages.RemoveAt(e.RowIndex);
                }
            }
        }
        ...
        {
            if (this.ErrorMessages.Count > 0 && MessageBox.Show("程式執行過程中有錯誤發生，確定要關閉程式？", "詢問", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
            {
                e.Cancel = true;
        ...
        {
            this.stopWorkingMessageHandler = true;
            this.workingMessageWait.Set();
        }
